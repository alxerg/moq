...
//             {{.Name}}Fn: func({{ .Arglist }}) {{.ReturnArglist}} {
...
func (mock *{{$obj.InterfaceName}}Mock) {{.Name}}({{.Arglist}}) {{.ReturnArglist}} {
	if mock.{{.Name}}Fn == nil {
		panic("moq: {{$obj.InterfaceName}}Mock.{{.Name}}Fn is nil but {{$obj.InterfaceName}}.{{.Name}} was just called")
	}
	callInfo := struct {
		{{- range .Params }}
		{{ .Name | Exported }} {{ .Type }}
...
{{- if .ReturnArglist }}
	return mock.{{.Name}}Fn({{.ArgCallList}})
{{- else }}
	mock.{{.Name}}Fn({{.ArgCallList}})
{{- end }}
}
